...
const Card  = ( { slot, icon, title }: Properties ) => {
    return (
        <Box as={ Tilt } width={ 'full' } maxWidth={ { base: 'full', sm: '240px', md: '240px' } } key={ slot } tiltReverse={ true }>
            <Box as={ motion.div } variants={ fadeIn('right', 'spring', slot * 0.5, 0.75 ) } padding={ '2px' } rounded={ '2xl' } backdropFilter={ 'auto' } backdropBlur={ '2px' } backgroundColor={ 'whiteAlpha.50' }>
                <ChakraCard height={ '17rem' } boxShadow={ 'dark-lg' } rounded={ '2xl' } backgroundColor={ 'transparent' }>
